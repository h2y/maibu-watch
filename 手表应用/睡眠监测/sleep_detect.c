...
static uint32_t get_sleep_level(uint8_t wday)
{
	switch (week_info[wday].sleepQuality)
	{
	case 0: return RES_BITMAP_LV_3;
	case 1: return RES_BITMAP_LV_2;
	case 2: return RES_BITMAP_LV_1;
	default: return RES_BITMAP_LV_0;
	}
}
...
static void update_display(void)
{
	...
	if (BGM_flag == 0)
	{
		...
		*p_frame_bmp = 0x0c1a1f1c;
		res_get_user_bitmap(array_num[weekday_sum[datetime.wday] / 60 / 10], &bmp_temp);
		create_layer_bmp(p_new_window, &bmp_temp, &frame_bmp);
...
static void getWeekSleepInfo(void)
{
	struct date_time datetime;
	app_service_get_datetime(&datetime);
