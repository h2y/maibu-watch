 ...
 *        Created:  2015 年 04 月 17 日 10 时 03 分 03 秒
 *
 * =====================================================================================
 *
...
/* 创建并显示图片图层，需要坐标值，得到 icon_key 的数组，数组需要的参数值，P_Window*/
...
void timer_callback(date_time_t tick_time, uint32_t millis, void *context)
{
	...
	if (g_timer_sec == 0)
	{
		...
		sprintf(param.main_title, "%d 分钟 ", g_timer_pre_set);
	...
	if (NULL != p_window)
	{
		...
		if (g_timer_sec == -1)
		{
			...
			if (p_layer != NULL)
			{
				...
				/* 设置文本图层新的文本内容并更新显示 */
				char text[10] = "";
				sprintf(text, "%d 分钟 ", g_timer_pre_set);
...
/*-------- 添加状态栏显示时间 -------*/
	...
	else if (g_timer_sec> 0)
	{
		...
		sprintf(text, " 计时 %d 分钟 ", g_timer_pre_set);
